...
public class ParseFirebaseData2 {
    private SettingsAPI set;

    public ParseFirebaseData2(Context context) {
        set = new SettingsAPI(context);
    }

    public List<Friend> getUserList(String userData,String idGmail) {
        ...
        for (String oneUser : userData.split("[}][,]")) {
            ...
            if (!set.readSetting("myid").equals(id) && idGmail.equals(id))
    ...
    public List<ChatMessage> getMessageListForUser(String msgData) {
        ...
        String receiverEmail = null, senderEmail = null;
        for (String msgInConv : msgData.split("[}][,]")) {
            ...
            for (String part : msgParts) {
                ...
                if (part.split("=")[0].trim().equals("senderemail"))
                    senderEmail = part.split("=")[1].trim();
            }
            tempMsg = new ChatMessage(senderEmail,receiverEmail,text, msgTime, receiverId, receiverName, receiverPhoto, senderId, senderName, senderPhoto);
            chats.add(tempMsg);
        }
